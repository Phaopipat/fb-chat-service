...
// ─── V110 · KB direct precedence guard ──────────────────────────────────────
// Skip KB direct lookup for high-risk customer intents (payment / cancel / modify)
// when LP stage is booking or won · falls through to AI gen + V107 stage guidance.
// Reference: 2026-06-18-V110-kb-precedence-guard-spec.md

// Thai keywords · substring match (Thai script U+0E00–U+0E7F won't false-positive into EN)
const HIGH_RISK_PAYMENT_KW_TH = [
  'มัดจำ', 'มัดจํา',
  'จ่าย', 'จ่ายเงิน', 'ชำระ', 'ชําระ',
  'โอน', 'โอนเงิน', 'โอนแล้ว', 'โอนไป',
  'สลิป', 'สลิ๊ป',
  'จ่ายหน้างาน', 'จ่ายเลย', 'จ่ายก่อน', 'จ่ายตอน', 'จ่ายทีหลัง',
  'เครดิต', 'เงินสด',
];

// EN keywords · word-boundary regex (prevents "playing"/"display"/"splay" false positives)
const HIGH_RISK_PAYMENT_RE_EN = /\b(?:pay|paid|paying|payment|payments|deposit|deposits|transfer|transferred|wire|remit|remitted|slip)\b/i;

function isPaymentIntent(text) {
  if (!text) return false;
  const raw = String(text);
  const lower = raw.toLowerCase();
  for (const kw of HIGH_RISK_PAYMENT_KW_TH) {
    if (lower.includes(kw)) return true;
  }
  return HIGH_RISK_PAYMENT_RE_EN.test(raw);
}

// Self-contained cancel detector · NO require to ai-reply.js (avoids circular dep)
// Mirrors V83_CANCEL_CONTEXT_RE semantics from ai-reply.js · maintained independently.
const HIGH_RISK_CANCEL_RE = /ยกเลิก|แคนเซิล|cancel(?:lation|led|s)?|refund|คืนเงิน|เลื่อน(?:วัน|จอง|booking|เข้าพัก|กำหนด)?|เปลี่ยน(?:วัน|จอง|booking|ห้อง|เดท|กำหนด)?|ขอเลื่อน|ขอเปลี่ยน|ย้ายวัน|postpone|reschedul(?:e[ds]?|ing)?|push back|push-back/i;

function isCancelIntent(text) {
  if (!text) return false;
  return HIGH_RISK_CANCEL_RE.test(String(text));
}

// Modify detector · requires topic anchor (date/room/booking/etc) · avoids bare "เปลี่ยนใจ"
const HIGH_RISK_MODIFY_RE_TH = /(?:เปลี่ยน|เลื่อน|ย้าย).*?(?:วัน|เดท|ห้อง|รูม|จำนวน|คน|กี่คน)/i;
const HIGH_RISK_MODIFY_RE_EN = /\b(?:change|move|swap|reschedul(?:e[ds]?|ing)?|postpone|amend|update)\b.*?(?:\bdate\b|\broom\b|\bbooking\b|\bnight\b|\bguest\b)/i;

function isModifyIntent(text) {
  if (!text) return false;
  const t = String(text);
  return HIGH_RISK_MODIFY_RE_TH.test(t) || HIGH_RISK_MODIFY_RE_EN.test(t);
}

const BOOKING_STAGES = new Set(['booking', 'won']);

let _v110Counters = {
  kbDirectSkipped_payment: 0,
  kbDirectSkipped_cancel: 0,
  kbDirectSkipped_modify: 0,
};

function isKbPrecedenceGuardEnabled() {
  return (process.env.KB_PRECEDENCE_GUARD_ENABLED || 'false').toLowerCase() === 'true';
}

function shouldSkipKBForIntent(msgText, profile) {
  if (!isKbPrecedenceGuardEnabled()) return null;
  if (!profile || typeof profile !== 'object') return null;
  if (!BOOKING_STAGES.has(profile.stage)) return null;

  if (isPaymentIntent(msgText)) {
    _v110Counters.kbDirectSkipped_payment++;
    return 'payment';
  }
  if (isCancelIntent(msgText)) {
    _v110Counters.kbDirectSkipped_cancel++;
    return 'cancel';
  }
  if (isModifyIntent(msgText)) {
    _v110Counters.kbDirectSkipped_modify++;
    return 'modify';
  }
  return null;
}

function getV110Counters() {
  return { ..._v110Counters };
}

function resetV110Counters() {
  _v110Counters = {
    kbDirectSkipped_payment: 0,
    kbDirectSkipped_cancel: 0,
    kbDirectSkipped_modify: 0,
...
// ─── KB cache invalidation (V110 bonus · separate from pricing cache) ────────
let _kbCacheInvalidated = false;
function invalidateKBCache() {
  _kbCacheInvalidated = true;
  console.log('[V110] KB cache invalidated (manual)');
}
...
async function readKB({ sheets, sheetId, category } = {}) {
  try {
    ...
    // V110 · honor manual invalidation from /admin/refresh-kb-cache
    if (_kbCacheInvalidated) {
      kbCacheStore = { data: [], at: 0 };
      _kbCacheInvalidated = false;
    }
...
// ─── tokenize ────────────────────────────────────────────────────────────────
function isV111TokenizerEnabled() {
  return (process.env.V111_TOKENIZER_ENABLED || 'false').toLowerCase() === 'true';
}

/**
 * Legacy tokenizer kept for production fallback.
 * Thai no-whitespace text becomes one large token; V111 fixes this behind a flag.
 */
...
const V111_GENERIC_THAI_TOKENS = new Set([
  'วันนี้', 'พรุ่งนี้', 'พรุ่ง', 'นี้',
  'ได้', 'ไหม', 'มั้ย', 'ยัง', 'ไง', 'อย่างไร', 'ยังไง',
  'หรือ', 'ครับ', 'คับ', 'ค่ะ', 'คะ', 'นะ', 'เลย',
  // Inquiry glue words that should not bridge unrelated KB entries.
  'อยาก', 'ทราบ', 'เรื่อง', 'สอบถาม', 'ขอ', 'ข้อมูล', 'รายละเอียด',
  'แบบ', 'ไหน', 'บ้าง', 'กี่', 'โมง', 'เท่า', 'ไหร่', 'การ',
  // Common helper/action words. Keep content words like "จ่าย" intact.
  'มี', 'ค่า', 'ใช้', 'ต้อง',
  // Thai transliteration of check-in often segments into two generic pieces.
  'เช็ค', 'อิน',
]);
...
function getThaiSegmenter() {
  if (_thaiSegmenter) return _thaiSegmenter;
...
function normalizeV111Token(token) {
  if (!token) return '';
  const normalized = String(token)
    .toLowerCase()
    .replace(/[^฀-๿a-z0-9]/g, '')
    .trim();
  // Normalize surcharge synonyms so "ค่าใช้จ่ายเพิ่ม" can match "ต้องบวก".
  if (normalized === 'เพิ่ม') return 'บวก';
  return normalized;
}

function shouldKeepV111Token(token) {
  if (!token || token.length < 2) return false;
  if (V111_GENERIC_THAI_TOKENS.has(token)) return false;
  return true;
}

/**
 * V111 hybrid tokenizer:
 * - Pure ASCII/EN keeps legacy whitespace tokenization.
 * - Thai/mixed text uses native Intl.Segmenter('th') word segmentation.
 * - Generic Thai particles/time/action glue words are dropped to avoid
 *   false 1.000 scores from chunks sharing only "พรุ่งนี้"/"ได้ไหม".
 */
function tokenizeV111(text) {
  if (!text) return new Set();
  const raw = String(text).toLowerCase();
  if (/^[a-z0-9\s]+$/i.test(raw)) return tokenizeLegacy(raw);

  const segmenter = getThaiSegmenter();
  if (!segmenter) return tokenizeLegacy(raw);

  const tokens = [];
  for (const part of segmenter.segment(raw)) {
    if (!part.isWordLike) continue;
    const normalized = normalizeV111Token(part.segment);
    if (shouldKeepV111Token(normalized)) tokens.push(normalized);
  }
  return new Set(tokens);
}
...
function tokenize(text) {
  if (isV111TokenizerEnabled()) return tokenizeV111(text);
...
function softMatch(a, b) {
  ...
  // V111 word segmentation makes broad substring fallback unnecessary and risky
  // for Thai. Keep exact/includes, but disable arbitrary common-substring scans.
  if (isV111TokenizerEnabled()) return false;
 ...
 */
async function lookupKB({ sheets, sheetId, customerMessage, topic, apiKey, today, profile } = {}) {
  if (!sheets || !sheetId || !customerMessage) return null;

  try {
    // V110 · High-risk intent precedence guard
    // Skip KB direct when (payment OR cancel OR modify) AND stage IN (booking, won)
    // Falls through to AI gen path with V107 stage guidance + V108 customer context.
    // Reference: 2026-06-18-V110-kb-precedence-guard-spec.md
    const skipReason = shouldSkipKBForIntent(customerMessage, profile);
    if (skipReason) {
      console.log(`[V110] KB direct skipped · reason=${skipReason} stage=${profile.stage} msg="${String(customerMessage).slice(0, 40)}"`);
      return null;
    }
...
module.exports = {
  ...
  _resetCache,
  getKBCacheStats,
  // V110 · KB direct precedence guard exports
  isPaymentIntent,
  isCancelIntent,
  isModifyIntent,
  shouldSkipKBForIntent,
  isKbPrecedenceGuardEnabled,
  getV110Counters,
  resetV110Counters,
  invalidateKBCache,
  _isPaymentIntent: isPaymentIntent,
  _isCancelIntent: isCancelIntent,
  _isModifyIntent: isModifyIntent,
  _shouldSkipKBForIntent: shouldSkipKBForIntent,
};
