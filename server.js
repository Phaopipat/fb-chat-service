...
async function initSheet() {
        ...
        try {
          ...
          // Create FB-Messages sheet if missing
          ...
          // Create FB-Config sheet if missing (stores lastPollTimestamp)
          if (!sheetNames.includes('FB-Config')) {
                      await sheets.spreadsheets.batchUpdate({
                                    spreadsheetId: SHEET_ID,
                                    requestBody: {
                                                    requests: [{
                                                                      addSheet: { properties: { title: 'FB-Config' } }
                                                    }]
                                    }
                      });
                      await sheets.spreadsheets.values.append({
                                    ...
                                    valueInputOption: 'RAW',
                                    requestBody: {
                                                    values: [['Key', 'Value'], ['lastPollTime', '']]
                                    }
                      });
                      console.log('📋 Created FB-Config sheet');
          }
        } catch (err) {
...
async function loadLastPollTime() {
        ...
        try {
                  ...
                  if (val) {
                              console.log(`📅 Loaded lastPollTime from sheet: ${val}`);
                              return val;
                  }
        } catch (err) {
                  console.error('Load lastPollTime error:', err.message);
        }
        return null;
}

async function saveLastPollTime(timestamp) {
        const sheets = await getSheetsClient();
        ...
        try {
                  ...
                  console.error('Save lastPollTime error:', err.message);
        }
}
...
async function loadProcessedIds() {
        ...
        try {
                  const res = await sheets.spreadsheets.values.get({
                              spreadsheetId: SHEET_ID,
                              range: 'FB-Messages!A2:J'
                  });
                  const rows = res.data.values || [];
                  const recent = rows.slice(-500);
                  let loaded = 0;
                  for (const row of recent) {
                              const ts = row[0] || '';
                              const senderId = row[2] || '';
                              const msg = (row[4] || '').substring(0, 50);
                              if (ts && senderId) {
                                            processedMessageIds.add(`${ts}|${senderId}|${msg}`);
                                            loaded++;
                              }
                  }
                  console.log(`📋 Pre-loaded ${loaded} message fingerprints from sheet`);
        } catch (err) {
                  console.error('Load processed IDs error:', err.message);
        }
}
...
function msgFingerprint(timestamp, senderId, message) {
        return `${timestamp}|${senderId}|${(message || '').substring(0, 50)}`;
}
...
async function pollFacebookMessages() {
  ...
  for (const conv of conversations) {
          ...
          if (conv.messages && conv.messages.data) {
                      for (const msg of conv.messages.data) {
                                    const fp = msgFingerprint(msg.created_time, msg.from.id, msg.message);
                                    if (processedMessageIds.has(msg.id) || processedMessageIds.has(fp)) continue;

                        const isPage = msg.from.id === pageId;
                                    const senderRole = isPage ? 'page' : 'customer';

                        processedMessageIds.add(msg.id);
                                    processedMessageIds.add(fp);
          ...
          if (conv.source === 'comment') {
                      const fp = msgFingerprint(conv.created_time, conv.from?.id, conv.message);
                      if (processedMessageIds.has(conv.id) || processedMessageIds.has(fp)) continue;
  ...
  console.log(`✅ Poll complete: ${newCount} new messages saved`);
        ...
        await saveLastPollTime(lastPollTime);
}
...
app.listen(PORT, async () => {
        ...
        console.log('————————————————————————');
          ...
          // Restore state from sheet to prevent duplicates after restart
          const savedPollTime = await loadLastPollTime();
                       if (savedPollTime) lastPollTime = savedPollTime;
